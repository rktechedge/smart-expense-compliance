...
# View for agent responses compatible with agent analytics query
resource "google_bigquery_table" "v_agent_response" {
  project             = var.project_id
  dataset_id          = google_bigquery_dataset.telemetry_dataset.dataset_id
  table_id            = "v_agent_response"
  description         = "View of final agent responses for analytics"
  deletion_protection = false

  view {
    query = <<EOF
SELECT
  content AS response_text,
  agent_name AS agent,
  timestamp,
  conversation_id,
  trace,
  span_id
FROM
  `${var.project_id}.${google_bigquery_dataset.telemetry_dataset.dataset_id}.completions_view`
WHERE
  message_type = 'output'
EOF
    use_legacy_sql = false
  ...
  depends_on = [
    google_bigquery_table.completions_view
  ]
}
